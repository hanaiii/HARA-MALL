...
public class DateUtil {
    ...
    public static void main(String[] args) {
        Date[] dates = getCurrentTimeArea(2);
    ...
    /**
     * 获取当前时间所处的间隔。
     * 如gap为6时，表示时间段有：[0,6)、[6,12)、[12,18)、[18,24)
     * @param gap 时间段之间的间隔，必须能被24整除
     * @return
     */
    public static Date[] getCurrentTimeArea(int gap) {
        if(24 % gap != 0){
            throw new RuntimeException("时间间隔错误！必须要能被24整除!");
        }
        // 凌晨
        Date now = new Date();
        Date date = toDayStartHour(now);
        // 每6个小时为一个时间段
        while(true){
            if (now.getTime() >= date.getTime() && now.getTime() < addDateHour(date, gap).getTime()){
                return new Date[]{date, addDateHour(date,gap)};
            }else{
                date = addDateHour(date, gap);
            }
        }
